...
_MSG_ONBOARDING = (
    "👋 Hello!\n\n"
    "To verify your phone number, please send the <b>6-digit verification code</b> "
    "you received upon completing your registration on the website.\n\n"
    "📌 Example: <code>123456</code>"
)
_MSG_INVALID_CODE = "❌ The code is incorrect, expired, or has already been used. Please check the code and try again."
_MSG_SUCCESS = (
    "✅ <b>Verification completed successfully!</b> 🎉\n\n"
    "Hello {name}, your phone number has been verified in the Shabbat Hosting system.\n"
...
def register(user_in: UserCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    new_user = User(
        ...
        telegram_verification_code=telegram_code,
        telegram_verification_expires_at=expires_at,
    )
    db.add(new_user)
    db.flush()

    profile_map = {
        UserType.HOST: HostProfile(user_id=new_user.id, city="Not Specified"),
        UserType.GUEST: GuestProfile(user_id=new_user.id),
    }
    if profile := profile_map.get(new_user.user_type):
        db.add(profile)
...
def get_me(current_user: User = Depends(get_current_user)):
    profile = (
        current_user.host_profile
        if current_user.user_type == UserType.HOST
        else current_user.guest_profile
    )
    return {**current_user.__dict__, "id": str(current_user.id), "profile": profile}
...
def verify_phone(data: VerifyPhoneRequest, db: Session = Depends(get_db)):
    ...
    user.telegram_verification_expires_at = None
    db.commit()
...
def telegram_webhook(update: dict, db: Session = Depends(get_db)):
    ...
    if not (text.isdigit() and len(text) == 6):
        send_telegram_message(chat_id, _MSG_ONBOARDING)
        return {"status": "ignored"}

    user = db.query(User).filter(
        User.telegram_verification_code == text,
        User.telegram_verification_expires_at > datetime.now(timezone.utc),
    ).first()
...
def update_host_profile(
    ...
    embed_text = " ".join(filter(None, [
        profile.city, profile.neighborhood, profile.religious_orientation, profile.free_text_notes
    ]))
    if embed_text:
...
def update_guest_profile(
    ...
    embed_text = " ".join(filter(None, [
        profile.skills_give_take, profile.food_preferences_allergies, current_user.biography
    ]))
    if embed_text:
